    ...
    import grails.converters.JSON  // 👈 Este import está correto
    import java.text.SimpleDateFormat

    class BootStrap {
    
        GrailsApplication grailsApplication  // ← Adicione esta linha
        def init = { servletContext ->
            ...
            println "=" * 60
            println ">>> JSON Date Marshaller configurado!"
            println "=" * 60
        ...
        private void criarPastaDocumentos() {
            try {
                // Usar caminho absoluto baseado no servlet context
                String appPath = System.getProperty('catalina.base') ?: '.'  // Tomcat
                String storagePath = "${appPath}/data/documents"
    
                // Ou usar caminho relativo simples
                // String storagePath = 'data/documents'
    
                File pasta = new File(storagePath)
    
                if (!pasta.exists()) {
                    boolean criada = pasta.mkdirs()
                    if (criada) {
                        println "📁 Pasta de documentos criada: ${pasta.absolutePath}"
                    } else {
                        println "❌ Não foi possível criar a pasta: ${pasta.absolutePath}"
                    }
                } else {
                    println "📁 Pasta de documentos já existe: ${pasta.absolutePath}"
                }
            } catch (Exception e) {
                println "❌ Erro ao criar pasta de documentos: ${e.message}"
            }
        }
        /**
